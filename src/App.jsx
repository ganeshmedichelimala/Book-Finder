  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        url.searchParams.set('fields', 'totalItems,items(id,volumeInfo(title,authors,imageLinks/thumbnail,previewLink))')
    ...
    <div className="min-h-full">
      <header className="border-b border-slate-800">
        <div className="mx-auto container-max px-4 py-5 flex items-center gap-4 justify-between">
          ...
          <a className="badge" href="https://developers.google.com/books" target="_blank" rel="noreferrer">API Docs</a>
        </div>
      </header>
      ...
      <main className="mx-auto container-max px-4 py-6 space-y-6">
        <div className="card p-4">
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="self-center text-sm text-slate-400">Results: <span className="font-medium text-slate-200">{totalItems}</span></div>
            {q && <div className="self-center text-sm text-slate-400 col-span-1 md:col-span-3">Query: <span className="font-medium text-slate-200">{q}</span></div>}
          </div>
        </div>

        <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {items.map(b => <BookCard key={b.id + b.title} book={b} />)}
        </section>

        {error && <div className="card p-6 text-center text-rose-300 border-rose-800">Failed to load: {error}</div>}
        {!loading && q && items.length === 0 && !error && (
          <div className="card p-6 text-center">No books found for this query.</div>
        )}
